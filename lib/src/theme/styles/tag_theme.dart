...
* @return:
* @author:          novice.cai
* @time:            2023/10/17 17:18
*/
class FNTagTheme extends InheritedTheme {
  const FNTagTheme({
    Key? key,
    required this.data,
  ...
  static Widget merge({
    ...
    return Builder(
      builder: (BuildContext context) {
        return FNTagTheme(
          key: key,
          data: FNTagTheme.of(context).merge(data),
          child: child,
        );
      },
    );
  ...
  final FNTagThemeData data;

  static FNTagThemeData of(BuildContext context) {
    final FNTagTheme? tagTheme =
        context.dependOnInheritedWidgetOfExactType<FNTagTheme>();
    return tagTheme?.data ?? FNTheme.of(context).tagTheme;
  }

  @override
  bool updateShouldNotify(FNTagTheme oldWidget) => data != oldWidget.data;

  @override
  Widget wrap(BuildContext context, Widget child) {
    return FNTagTheme(data: data, child: child);
  }
...
class FNTagThemeData with Diagnosticable {
  factory FNTagThemeData({
    EdgeInsets? padding,
    Color? textColor,
    double? fontSize,
    double? borderRadius,
    ...
    final double fontFinalSize = fontSize ?? FNColors.fontSizeSm;
    return FNTagThemeData.raw(
      padding: padding ??
          const EdgeInsets.symmetric(
              vertical: 0, horizontal: FNColors.paddingBase),
      textColor: textColor ?? FNColors.white,
      fontSize: fontFinalSize,
      borderRadius: borderRadius ?? 2.0,
      lineHeight: lineHeight ?? (16.0 / fontFinalSize),
      mediumPadding: mediumPadding ??
          const EdgeInsets.symmetric(vertical: 2.0, horizontal: 6.0),
      largePadding: largePadding ??
          const EdgeInsets.symmetric(
              vertical: FNColors.paddingBase, horizontal: FNColors.paddingXs),
      largeBorderRadius: largeBorderRadius ?? FNColors.borderRadiusMd,
      largeFontSize: largeFontSize ?? FNColors.fontSizeMd,
      roundBorderRadius: roundBorderRadius ?? FNColors.borderRadiusMax,
      dangerColor: dangerColor ?? FNColors.backgroundDanger,
      primaryColor: primaryColor ?? FNColors.primary,
      successColor: successColor ?? FNColors.backgroundSuccess,
      warningColor: warningColor ?? FNColors.backgroundWarning,
      defaultColor: defaultColor ?? FNColors.background,
      plainBackgroundColor: plainBackgroundColor ?? FNColors.white,
    );
  }

  const FNTagThemeData.raw({
  ...
  FNTagThemeData merge(FNTagThemeData? other) {
    if (other == null) {
      return this;
    }
    return copyWith(
  ...
  static FNTagThemeData lerp(FNTagThemeData? a, FNTagThemeData? b, double t) {
    return FNTagThemeData(
  ...
  bool operator ==(Object other) {
    ...
    return other is FNTagThemeData &&
